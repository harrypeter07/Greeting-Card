...
const TEMPLATES = [
  { id: "classic", name: "Classic", background: "bg-gradient-to-r from-green-100 to-green-300" },
  { id: "modern", name: "Modern", background: "bg-gradient-to-r from-blue-100 to-blue-300" },
  { id: "festive", name: "Festive", background: "bg-gradient-to-r from-yellow-100 to-yellow-300" },
]
...
export function CardEditor() {
  ...
  const [message, setMessage] = useState("Eid Mubarak! Wishing you joy, peace, and prosperity.")
  ...
  const handleSaveCard = async () => {
    if (status !== "authenticated") {
      toast({
        ...
        description: "You need to sign in to save your card",
        variant: "destructive",
      })
      return
    }
  ...
  const getTemplateBackground = () => {
    return TEMPLATES.find((t) => t.id === template)?.background || TEMPLATES[0].background
  }
        ...
        <Card className="overflow-hidden">
          <CardContent className={`p-0 ${getTemplateBackground()}`}>
            <div ref={cardRef} className="aspect-[4/3] p-8 flex flex-col items-center justify-center text-center">
              ...
              <h2 className="animate-element text-2xl font-bold mb-4">{title}</h2>
              <p className="animate-element text-lg">{message}</p>

              {template === "classic" && <div className="animate-element mt-6 text-3xl">🕌</div>}
